...
contains 
  !
  subroutine setnum( frac , maxstep , backwardOnly) bind(c)
    !
    ! This subroutine controls the numerical integration parameters
    !
    real( kind = rkind )   , intent(in),value :: frac
    integer( kind = ikind ), intent(in),value :: maxstep
    integer( kind = ikind ), intent(in),value :: backwardOnly
	
	if (backwardOnly > 0) then
		num_backwardForward = 1
	else
		num_backwardForward = 2
	endif
  ...
  subroutine setbndlist( inbndlist , n ) bind(c)
    ...
    integer(kind=iKind),intent(in),value   :: n
    !
  ...
  subroutine calc( matrix, xp,yp,np, freq, nfreq,angles,nang, nsubrays ) bind(c)
    ...
    ! Input variables
    !-----------------
    integer( kind = iKind),intent(in)  ,value  :: nsubrays
    integer( kind = iKind),intent(in)  ,value  :: nang
    ...
    integer( kind = iKind),intent(in) ,value   :: np
    real(    kind = rKind),intent(in)    :: freq(nfreq)
    integer( kind = iKind),intent(in) ,value   :: nfreq
  ...
  subroutine fullcircle( matrix,nout,xp,yp,freq,angles,nang,maxrays) bind(c)
    ...
    do isec = 1, nang
       ...
       do iDirection =  1,num_backwardForward           
          !
          ...
          if (  inBndlist( int(outpar(iRot)) - 10 ) ) then
             ...
             if ( found ) then
                !
!   			    write( msg , '(f8.3, f8.3, i3,i3  )' ) inpar(3),outpar(3), iang(1),iang(2)
!    		    call wlog( msg , 0 ) 				
  ...
  subroutine setbat( di , xi, yi,nxi,nyi ) bind(c)
    ...
    integer(kind=iKind),value    :: nxi
    integer(kind=iKind),value    :: nyi
  ...
  subroutine setcont( d ) bind(c)
    !
    real(kind=rKind)   ,intent(in), value :: d
    !
    cont_d = d
    cont_set = .true.
